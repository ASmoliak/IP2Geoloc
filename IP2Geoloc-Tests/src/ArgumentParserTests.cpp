...
TEST(ArgumentParserTests, IPv4_Result_Validity)
{
	const char *valid_resolve_arguments[] = { PROGARM_PATH, IP_ARGUMENT, VALID_TARGET_IP };
	int valid_argument_count = 3;

	ArgumentParser parser(valid_argument_count, valid_resolve_arguments);
...
TEST(ArgumentParserTests, Bad_Argument_Count)
{
	const char *valid_resolve_arguments[] = { PROGARM_PATH, IP_ARGUMENT, VALID_TARGET_IP };
	int invalid_argument_count = 2;

	EXPECT_ANY_THROW(ArgumentParser(invalid_argument_count, valid_resolve_arguments))
		<< "Our code did not throw an exception on invalid argument count";
}

TEST(ArgumentParserTests, Self_Result_Validity)
{
...
TEST(ArgumentParserTests, Conflicting_Option_Detection)
{
	...
	EXPECT_THROW(ArgumentParser(three_arguments, conflicting_arguments1), std::invalid_argument) << error_string;
	EXPECT_THROW(ArgumentParser(three_arguments, conflicting_arguments2), std::invalid_argument) << error_string;
	EXPECT_THROW(ArgumentParser(three_arguments, conflicting_arguments3), std::invalid_argument) << error_string;
	EXPECT_THROW(ArgumentParser(three_arguments, conflicting_arguments4), std::invalid_argument) << error_string;
	EXPECT_THROW(ArgumentParser(three_arguments, conflicting_arguments5), std::invalid_argument) << error_string;
	EXPECT_THROW(ArgumentParser(three_arguments, conflicting_arguments6), std::invalid_argument) << error_string;
	...
	EXPECT_THROW(ArgumentParser(four_arguments, conflicting_arguments7), std::invalid_argument) << error_string;
	EXPECT_THROW(ArgumentParser(four_arguments, conflicting_arguments8), std::invalid_argument) << error_string;
	EXPECT_THROW(ArgumentParser(four_arguments, conflicting_arguments9), std::invalid_argument) << error_string;
	EXPECT_THROW(ArgumentParser(four_arguments, conflicting_arguments10), std::invalid_argument) << error_string;
	EXPECT_THROW(ArgumentParser(four_arguments, conflicting_arguments11), std::invalid_argument) << error_string;
	EXPECT_THROW(ArgumentParser(four_arguments, conflicting_arguments12), std::invalid_argument) << error_string;
}